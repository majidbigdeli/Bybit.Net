...
{
    /// <inheritdoc cref="IBybitSocketClientSpotApiV1"/>
    public class BybitSocketClientSpotApiV1 : BybitSocketClientBaseSpotApi, IBybitSocketClientSpotApiV1
    {
        internal BybitSocketClientSpotApiV1(ILogger logger, BybitSocketOptions options)
            : base(logger, options.Environment.SocketBaseAddress, options, options.SpotV1Options)
        {
        }
        ...
        public async Task<CallResult<UpdateSubscription>> SubscribeToTradeUpdatesAsync(string symbol, Action<DataEvent<BybitSpotTradeUpdate>> handler, CancellationToken ct = default)
        {
            ...
            return await SubscribeAsync(
                BaseAddress.AppendPath("/spot/quote/ws/v1"),
        ...
        /// <inheritdoc />
        public async Task<CallResult<UpdateSubscription>> SubscribeToOrderBookUpdatesAsync(string symbol, Action<DataEvent<ByBitDiffSpotOrderBookUpdate>> handler, CancellationToken ct = default)
        {
            var internalHandler = new Action<DataEvent<JToken>>(data =>
            {
                var internalDataArray = data.Data["data"];
                ...
                var desResult = Deserialize<ByBitDiffSpotOrderBookUpdate>(internalData);
                if (!desResult)
                {
                    _logger.Log(LogLevel.Warning, $"Failed to deserialize {nameof(ByBitDiffSpotOrderBookUpdate)} object: " + desResult.Error);
                    return;
                }
        ...
        public async Task<CallResult<UpdateSubscription>> SubscribeToLeverageTokenUpdatesAsync(string symbol, Action<DataEvent<BybitSpotLeverageUpdate>> handler, CancellationToken ct = default)
        {
            var internalHandler = new Action<DataEvent<JToken>>(data =>
            {
                ...
                if (!desResult)
                {
                    _logger.Log(LogLevel.Warning, $"Failed to deserialize {nameof(BybitSpotLeverageUpdate)} object: " + desResult.Error);
                    return;
                }
